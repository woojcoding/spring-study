...
-  적용 대상이 클래스이기에 프록시 모드로 타겟 클래스 설정을 해주었다.
-  이렇게 되면 가짜 프록시 객체를 생성하고 컨테이너 생성 후 이 가짜 프록시 객체를 의존관계 주입 해준다.
- HTTP request와는 상관 없게 미리 주입이 가능해진다.
- 스프링 컨테이너가 이 클래스를 상속받은 가짜 프록시 객체를 CGLIB 이라는 바이트코드 조작 라이브러리를 사용하여 생성한다.
- 가짜 프록시 객체는 실제 HTTP request가 오게 되면 진짜 빈을 요청하는 위임 로직이 들어있다.
- 클라이언트가 로직을 호출하게 되면 가짜 프록시 객체의 메서드를 호출하고 
- 가짜 프록시 객체가 진짜 request 스코프 빈의 메서드를 호출하는 위임 로직이 들어 있는 것이다.
- 프록시 객체 덕분에 클라이언트는 싱글톤 빈을 사용하듯 편리하게 request 스코프 빈을 사용 가능하다.
- 프록시 객체는 부모 클래스를 상속받았기에 다형성을 적용할 수 있다.
- 꼭 웹 스코프가 아니어도 프록시는 사용 가능하다.

정리
- Provider와 프록시를 사용하여 진짜 객체 조회를 꼭 필요한 시점까지 지연처리 하였다.
- 마치 싱글톤을 사용하는 것 같지만 새로운 인스턴스가 반환되기에 주의해서 사용해야 한다.
- 유지보수를 위해 꼭 필요한 곳에서만 웹 스코프를 사용해야 한다. 
